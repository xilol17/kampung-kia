...
// ==========================================
export const checkStudentProgressAction = async (userId: string, currentSemester: string) => {
  logger.info(`[Action] Checking academic progress for student ${userId}`);
  ...
  const currentEnrollments = allEnrollments.filter(
    e => e.semester === currentSemester && ['PENDING_PA', 'APPROVED'].includes(e.status)
  );
