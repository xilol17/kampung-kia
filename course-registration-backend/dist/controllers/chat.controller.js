...
const getGenAIInstance = () => {
    ...
    keyIndex++;
    return new generative_ai_1.GoogleGenerativeAI(currentKey);
};
    ...
    `;
      ...
      `;
        ...
        try {
            ...
            try {
                ...
                groqSuccess = true;
                logger_1.logger.info(`[AI Engine] Groq 70B 大脑成功解析意图！`);
            }
            catch (groqError) {
                logger_1.logger.error(`[AI Engine] Groq 70B 解析失败:`, groqError.message);
            }
            // 2. 替补队员：Gemini Lite 
            if (!groqSuccess) {
                logger_1.logger.warn("[AI Engine] 切换至 Gemini 备用大脑...");
            ...
            logger_1.logger.error(`[AI Engine] 所有 AI 引擎全部崩溃:`, allAIError.message);
            const currentTimetable = await (0, course_service_1.getStudentTimetable)(studentId, activeSemester);
            res.status(200).json({
        ...
        const sanitizeCourseCodes = (rawCode) => {
            if (!rawCode)
                return [];
            if (Array.isArray(rawCode))
                return rawCode.map(c => c.toUpperCase().replace(/[^A-Z0-9]/g, ''));
            ...
            switch (intent) {
                case 'REGISTER_COURSE':
                    for (const cleanCode of codesToProcess) {
                        const res = await (0, course_service_1.registerCourseAction)(studentId, cleanCode, activeSemester, prefs);
                ...
                case 'DROP_COURSE':
                    for (const cleanCode of codesToProcess) {
                        const res = await (0, course_service_1.dropCourseAction)(studentId, cleanCode, activeSemester);
                ...
                case 'SWAP_COURSE':
                    for (const cleanCode of codesToProcess) {
                        const res = await (0, course_service_1.swapCourseAction)(studentId, cleanCode, activeSemester, data.targetSection || "", prefs);
                ...
                case 'SEARCH_COURSE':
                    for (const cleanCode of codesToProcess) {
                        const res = await (0, course_service_1.searchCourseAction)(cleanCode, activeSemester);
                        executionFeedback += `\n\n🔍 查课 ${cleanCode} 结果：\n${res.message}`;
                        // 🌟 完美运行的核心：安全地把数据拼接入数组
                    ...
                    break;
                case 'RECOMMEND_PLAN':
                    const recRes = await (0, course_service_1.recommendPlanAction)(studentId, activeSemester);
                    if (recRes.success) {
                        executionFeedback += `\n✅ 已生成【智能选课推荐卡片】。`;
                    ...
                    break;
                case 'CHECK_PROGRESS':
                    const progRes = await (0, student_service_1.checkStudentProgressAction)(studentId, activeSemester);
                    if (progRes.success) {
                        executionFeedback += `\n✅ 已生成【毕业学分动态进度看板】。`;
                ...
                case 'CHECK_PREREQUISITE':
                    for (const cleanCode of codesToProcess) {
                        const preRes = await (0, course_service_1.checkPrerequisiteAction)(cleanCode);
                        if (preRes.success && preRes.data) {
                            const listStr = preRes.data.hasPrerequisites
                                ? preRes.data.prerequisites.map((p) => `【${p.prerequisiteName} (${p.prerequisiteCode})】`).join('、')
                                : '无前置科目门槛。';
      ...
      `;
            try {
                ...
                logger_1.logger.error("[AI Mouth] 二次润色失败，回退到原始缝合文本", pass2Error);
                finalNaturalReply = `${aiResponse.reply}\n\n⚙️ 统筹执行反馈：${executionFeedback}`;
            }
        }
        let currentTimetable;
        if (needTimetable) {
            currentTimetable = await (0, course_service_1.getStudentTimetable)(studentId, activeSemester);
        }
        res.json({
        ...
        logger_1.logger.error("AI 脑短路了:", error);
