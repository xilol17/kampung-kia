...
exports.manualDropApi = exports.manualRegisterApi = exports.getDashboardIntel = exports.searchCoursesApi = exports.getCourseStructure = exports.getMyTimetable = void 0;
...
const getMyTimetable = async (req, res) => {
    try {
        ...
        const timetable = await (0, course_service_1.getStudentTimetable)(studentId, activeSemester);
        res.status(200).json({
            success: true,
        ...
        logger_1.logger.error("Error fetching timetable:", error);
...
const getCourseStructure = async (req, res) => {
    try {
        ...
        const passedCourseCodes = new Set(student.enrollments.filter(e => e.status === 'PASSED').map(e => e.section.courseCode));
        ...
        catalogItems.forEach((item) => {
            ...
            if (!passedCourseCodes.has(item.courseCode)) {
                // 这学期正在上吗？
                const isCurrent = student.enrollments.some((e) => e.section.courseCode === item.courseCode && ['PENDING_PA', 'APPROVED'].includes(e.status));
                // 🌟 核心修复：这门课是不是曾经挂过？
                const isFailed = student.enrollments.some((e) => e.section.courseCode === item.courseCode && e.status === 'FAILED');
        ...
        logger_1.logger.error("Error fetching course structure:", error);
        res.status(500).json({ success: false, message: "Server error" });
    }
};
...
const searchCoursesApi = async (req, res) => {
    try {
        ...
        logger_1.logger.error("Error searching courses:", error);
        res.status(500).json({ success: false, message: "Server error" });
    }
};
exports.searchCoursesApi = searchCoursesApi;
const getDashboardIntel = async (req, res) => {
    try {
        const studentId = req.user.userId;
        const settings = await prisma_1.prisma.systemSetting.findFirst();
        const activeSemester = settings?.activeSemester || '2526-SEM1';
        ...
        const nearlyFullSection = await prisma_1.prisma.section.findFirst({
            where: { semester: activeSemester },
        ...
        logger_1.logger.error("Error fetching dashboard intel:", error);
        res.status(500).json({ success: false, message: "Server error" });
    }
};
exports.getDashboardIntel = getDashboardIntel;
const manualRegisterApi = async (req, res) => {
    try {
        const studentId = req.user.userId;
        ...
        const targetCourse = await prisma_1.prisma.course.findUnique({ where: { courseCode } });
        ...
        logger_1.logger.info(`[Manual Action] Student ${studentId} manually registered for ${courseCode} Sec ${sectionNumber}.`);
        res.status(200).json({
            success: true,
        ...
        logger_1.logger.error("Error in manual registration:", error);
        res.status(500).json({ success: false, message: "Server error" });
    }
};
exports.manualRegisterApi = manualRegisterApi;
const manualDropApi = async (req, res) => {
    try {
        const studentId = req.user.userId;
        ...
        logger_1.logger.error("Error in manual drop:", error);
        res.status(500).json({
            success: false,
...
exports.manualDropApi = manualDropApi;
